<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $stmt = $pdo->prepare('SELECT * FROM packages WHERE location LIKE :search OR district LIKE :search AND package_status = :package_status');
        ...
        $stmt = $pdo->prepare('SELECT * FROM agencies WHERE agency_name LIKE :search OR agency_address LIKE :search AND agency_status = :agency_status');
        $stmt->execute([':search'=> "%". $search ."%",
  ...
  <div class="container">
    ...
    <p class="lead">
        <form action="search.php" method="post" class="input-group">
            <input type="text" name="search" value="<?php echo $search; ?>" id="" placeholder="Search for Agencies or Packages" class="form-control col-md-4" style="border-top-right-radius: 3px;  border-bottom-right-radius: 3px;">
                ...
                <div class="row g-0">
                    ...
                    <div class="col-md-8">
                        <div class="card-body pt-0">
                            <h4 class="card-title text-info"><?php echo $package['package_name']; ?>
                            <?php 
                                //read package date data
                                $date = readPackageDates($package['package_id']);
                                if(!empty($date) && $date['status'] == 'booking off'){
                                    echo '<span class="badge rounded-pill ml-1 status" style="color:red;">'. ucwords($date['status']) .'</span>';
                                }
                            ?>
                            </h4>
                            <h5 class="font-italic text-info" style="font-size: .85rem;"><span class="mr-1"><i class="fas fa-map-marker-alt"></i></span><?php echo $package['location'] .', '. $package['district']; ?></h5>
                            ...
                            <div>
                                <p class="text-muted" >
                                   <h5 style="color:green; display:inline;">Le <?php echo $package['budget_price']; ?> <span style="color:blue; display:inline;"> per person </span></h5>
