    ...
    if(isset($_GET['edit'])){
        if(isset($_POST['update_employee'])){
            ...
            if(!empty($employee_contact)){
                $pattern = "/^(?:(?:\+|00)[1-9]\d{0,2}\x20?)?(?:\(\d{1,4}\)|\d{1,4})(?:\x20|-)?\d{1,4}(?:\x20|-)?\d{1,3}$/";
...
<div class="container">
    ...
    <form action="" method="post" class="col-md-8">
        <div class="form-group pb-2">
            ...
            <input type="text" class="form-control" id="" value="<?php echo $employee['employee_firstname']; ?>" name="employee_firstname" required?>
        </div>
        <div class="form-group p-2">
            <label for="employee_firstname">Employee's LastName *</label>
            <input type="text" class="form-control" id="" value="<?php echo $employee['employee_lastname']; ?>" name="employee_lastname" required?>
        </div>
        <div class="form-group p-2">
            <label for="employee_email">Email address *</label>
            <input type="email" class="form-control" id="" value="<?php echo $employee['employee_email']; ?>" name="employee_email" required?>
        </div>
        <div class="form-group p-2">
        ...
        <div class="form-group p-2">
            ...
            <select name="agency_role" id="" class="custom-select">
                <option value="<?php echo $employee['role'] ?>" requied><?php echo $employee['role'] ?></option>
                <?php
                    if($employee['role'] == 'Senior Staff'){
                        echo '<option value="Junior Staff">Junior Staff</option>';
                    }else{
                        echo '<option value="Senior Staff">Senior Staff</option>';
                    }
                ?>
            </select>
        </div>
